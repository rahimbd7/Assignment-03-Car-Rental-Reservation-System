...
const loginUser = async function (payload: IAuth) {
    ...
    if (!await UsersModel.isPasswordMatched(payload?.password, user?.password as string)) {
    ...
    const accessToken = createToken(jwtPayLoad,config.jwt_access_secret as string,config.access_token_expiresIn as string);
    return {
        data:user,
        accessToken
    }
