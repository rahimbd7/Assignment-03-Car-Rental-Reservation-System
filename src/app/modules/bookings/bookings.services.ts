  ...
  try {
    ...
    const populatedBooking = await BookingsModel.findById(newBooking[0]._id)
      ...
      .populate({
        path: 'carId',
        select:
          'name description color isElectric status features pricePerHour isDeleted createdAt updatedAt',
        model: 'cars',
      })
      .lean()
...
const getAllBookingsFromDB = async (
  payload: Partial<Pick<IBookings, 'carId' | 'date'>>,
) => {
